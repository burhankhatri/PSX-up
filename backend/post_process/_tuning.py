...
from backend.post_process._shared import direction_from_change_pct  # noqa: E402
...
PREDICTION_LOG_PATH = Path(__file__).resolve().parent.parent.parent / "data" / "prediction_logs" / "prediction_log.json"
...
def _fetch_actual_on_or_after(symbol: str, date_str: str, cache: Dict[Tuple[str, int, int], List[Dict]]) -> Tuple[Optional[float], Optional[str]]:
    ...
    if local_key not in cache:
        local_path = Path(__file__).resolve().parent.parent.parent / "data" / f"{symbol}_historical_with_indicators.json"
